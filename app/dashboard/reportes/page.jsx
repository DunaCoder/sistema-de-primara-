...
export default function ReportesPage() {
  ...
  useEffect(() => {
    ...
    async function cargarSecciones() {
      try {
        ...
        if (!isMounted) return;
        const lista = res.success ? res.secciones : [];
        setSecciones(lista);
        if (lista.length > 0) {
          setGradoSeccion(String(lista[0].idGradoSeccion));
        }
      } catch (error) {
  ...
  // Cargar estudiantes y sus evaluaciones al cambiar sección o lapso
  useEffect(() => {
    ...
    async function cargarEstudiantes() {
      setLoading(true);
      try {
        // Pasamos null para materiaId para obtener todas las materias
        const result = await obtenerEstudiantesYNotas(gradoSeccion, null, lapso);
        if (!isMounted) return;
        const lista = result.success ? result.data : [];
        setEstudiantes(lista);
        if (lista.length > 0) {
          setEstudianteSeleccionado(lista[0]);
        } else {
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-white p-5 rounded-lg shadow border border-gray-200 print:hidden">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Boletines Informativos</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                const est = estudiantes.find(i => i.idInscripcion === Number(e.target.value));
        ...
        <div className="bg-white p-8 rounded-lg shadow-md border border-gray-300 print:shadow-none print:border-none print:p-0">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-gray-50 p-4 rounded-md mb-6 border border-gray-200 text-sm">
            <div>
              ...
              <span className="font-bold text-gray-800">{estudianteSeleccionado.nombre}</span>
            </div>
            <div>
            ...
            <tbody>
              {estudianteSeleccionado.evaluaciones && estudianteSeleccionado.evaluaciones.length > 0 ? (
                estudianteSeleccionado.evaluaciones.map((ev, index) => (
                  <tr key={index}>
                    <td className="border border-gray-300 p-3 font-semibold text-gray-800">{ev.materia}</td>
          ...
          <div className="grid grid-cols-2 gap-12 pt-16 text-center text-xs font-semibold text-gray-700">
            <div>
              <div className="border-t border-gray-400 pt-2 w-3/4 mx-auto">Docente de Aula</div>
            </div>
            <div>
              <div className="border-t border-gray-400 pt-2 w-3/4 mx-auto">Director(a) / Sello del Plantel</div>
            </div>
          </div>
        </div>
      ) : (
