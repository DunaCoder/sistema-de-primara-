...
import {
  obtenerEstatusCargaDocente,
  obtenerBoletinesMasivosPorSeccion,
} from "@/actions/coordinacionCierre";
    ...
    <>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200 pb-4">
          <div>
            <h1 className="text-xl font-bold text-slate-800 uppercase">
              Cierre Académico y Control de Boletines
            </h1>
            <p className="text-xs text-slate-500">
              Supervisión de notas cargadas por los docentes e impresión masiva
              por sección
            </p>
          </div>
              ...
              >
                ...
                <div className="flex justify-between items-center text-xs text-slate-500 pt-2">
                  <span>
                    ...
                    <strong>
                      {item.estudiantesConNotas} / {item.totalInscritos}
                    </strong>
                  </span>
                  <button
                    disabled={
                      item.estudiantesConNotas === 0 ||
                      imprimiendoId === item.idGradoSeccion
                    }
      ...
      <div className="hidden print:block p-4">
        {boletinesImpresion.map((b) => (
          <div
            key={b.idInscripcion}
            className="border-b-2 border-slate-900 pb-6 mb-8 [page-break-after:always]"
          >
            <div className="text-center mb-4">
              ...
              <p className="text-xs font-semibold text-slate-600">
                BOLETÍN INFORMATIVO DE EVALUACIÓN - LAPSO {lapso}
              </p>
            </div>
            ...
            <div className="flex justify-between text-xs mb-4 font-semibold border-y py-2 border-slate-200">
              <div>
                <p>
                  Estudiante: <span className="font-bold">{b.estudiante}</span>
                </p>
                <p>
                  Cédula Escolar: <span className="font-bold">{b.cedula}</span>
                </p>
              </div>
              <div className="text-right">
                <p>
                  Grado y Sección: <span className="font-bold">{b.grado}</span>
                </p>
              </div>
            </div>
              ...
              <thead>
                <tr className="bg-slate-100">
                  <th className="border border-slate-300 p-2 text-left">
                    Área de Formación / Materia
                  </th>
                  <th className="border border-slate-300 p-2 text-center w-16">
                    Literal
                  </th>
                  <th className="border border-slate-300 p-2 text-left">
