...
import { obtenerUsuarios, obtenerRoles, crearUsuarioAction } from '../../actions/usuarios';
...
export default function UsuariosPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (res?.success) {
      ...
      setFormData({ username: '', password: '', idRol: '', cedula: '', nombre: '', apellido: '', motivoResguardo: '' });
      setTab('listado');
      cargarDatos();
    } else {
    ...
    <div className="p-6 max-w-5xl mx-auto text-slate-800">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">Administración de credenciales e identidades del sistema</p>
        </div>
      </div>
      ...
      {cargando ? (
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-slate-200">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {usuarios.map((u) => (
                <tr key={u.id} className="text-sm text-slate-700 hover:bg-slate-50/80 transition-colors">
                  ...
                  <td className="p-4 font-mono text-xs text-slate-600">{u.cedula}</td>
                  <td className="p-4">
        ...
        <form onSubmit={handleSubmit} className="bg-white p-8 rounded-xl shadow-sm border border-slate-200 max-w-3xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                ...
                <select
                  ...
                  className="w-full border border-slate-300 bg-white text-slate-900 p-2.5 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all shadow-sm"
                >
                  <option value="" className="text-slate-400">Seleccione un rol...</option>
                  {roles.map((r) => (
                    <option key={r.idRol} value={r.idRol} className="text-slate-900">
                      {r.nombre}
                    </option>
                  ))}
                </select>
