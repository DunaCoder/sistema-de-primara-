  ...
  useEffect(() => {
    ...
    async function cargarMatriculaBD() {
      try {
        ...
        // Se llama directo a la Server Action sin pasar objetos de señal HTTP
            ...
            <tbody className="divide-y divide-gray-100 text-sm text-gray-700">
              {cargando || isPending ? (
                <tr>
                  ...
                  >
                    <span className="inline-block animate-pulse">
                      Cargando nómina desde la base de datos...
                    </span>
                  </td>
                </tr>
              ) : error ? (
                ...
                filtrados.map((item, index) => (
                  <tr
                    key={item.id || item.cedula || index}
                    className="hover:bg-slate-50 transition-colors"
                  >
                    <td className="p-4 font-semibold text-slate-900">
                      {item.cedula || "N/A"}
                    </td>
                    <td className="p-4 font-medium uppercase text-slate-800">
                      {item.estudiante || "N/A"}
                    </td>
                    <td className="p-4 text-slate-600">
                      {item.gradoSeccion || "N/A"}
                    </td>
                    <td className="p-4 text-slate-600">
                      {item.representante || "N/A"}
                    </td>
                  </tr>
                ))
              ) : (
