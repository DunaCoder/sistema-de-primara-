...
export default function GestionUsuariosPage() {
  ...
  const saveChanges = async (id) => {
    ...
    if (!res?.success) {
      Swal.fire(
        "Error",
        res?.error || "No se pudo actualizar el usuario",
        "error",
      );
      setSavingId(null);
      return;
    ...
    const rolSeleccionado = roles.find(
      (r) => String(r.idRol) === String(editForm.idRol),
    );
  ...
  const docentesCount = usuarios.filter((u) =>
    u.rol?.toString().toLowerCase().includes("docente"),
  ).length;
  const secretariaCount = usuarios.filter((u) =>
    u.rol?.toString().toLowerCase().includes("secretaria"),
  ).length;
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Gestión de accesos a la plataforma, asignación de roles e
                      ...
                      >
                        ...
                        <td className="p-4 text-center">
                          {isEditing ? (
                            ...
                            <span
                              className={`inline-block text-xs px-2.5 py-1 rounded-md font-semibold border ${getRolBadge(usr.rol)}`}
                            >
                              {usr.rol}
                            </span>
                          )}
